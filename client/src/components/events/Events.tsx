...
export function EventsOld() {
  ...
  useEffect(() => {
    if (selectedEventId) {
      ...
      const identifier = (events?.events?.find((e: any) => e._id === selectedEventId) as any)?.slug || selectedEventId;
  ...
  return (
    <div className="min-h-screen py-6 sm:py-12 mb-48 px-6 sm:px-12 lg:px-16 animate-in fade-in duration-500 font-inter animate-slide-up ">
      <style>{`
        /* Custom scrollbars for different sections */
      ...
      <div className="max-w-7xl mx-auto">
        <header className="mb-10 px-2">
          <h1 className="text-2xl sm:text-3xl font-[300] text-gray-900 tracking-tight">Explore Events</h1>
          <p className="text-gray-500 mt-1.5 text-[12px] sm:text-[15px]">Handpicked experiences for you</p>
        </header>
        {/* Filters */}
      <div className="rounded-[24px] mb-16">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
          ...
          <div className="flex-1 min-w-0 flex gap-2">
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                onKeyPress={handleSearchKeyPress}
                className="w-full text-[14px] pl-4 pr-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none"
              />
            {/* Search Button */}
            <button onClick={handleSearch} className="p-2 bg-white border border-gray-100 rounded-lg text-gray-400 hover:text-gray-900 transition-colors">
              <Search size={18} />
        ...
        {activeFilters.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-4">
            {activeFilters.map(([key, value]) => {
              ...
              return (
                <div key={key} className="flex items-center gap-1 px-3 py-1 bg-indigo-50 text-indigo-700 rounded-full text-sm">
                  <span>{label as string}</span>
                  <button
                    onClick={() => clearFilter(key as keyof EventFilters)}
        ...
        {/* Show trending and featured sections only when no filters are active */}
        {activeFilters.length === 0 ? (
          <>
            ...
            {trendingEvents.length > 0 && (
              <div className="mb-32">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-xl sm:text-2xl font-toroka font-medium text-gray-900">Trending Now</h2>
                  <span className="text-sm text-gray-500 font-medium">🔥 Hot picks based on popularity</span>
                </div>
                <div className="overflow-x-auto pb-4 trending-scrollbar">
                  <div className="flex gap-6 min-w-max">
                    {trendingEvents.map((event: any, idx: number) => (
                      <div key={`trending-${event._id}-${idx}`} className="flex-shrink-0 w-72">
                        <EventCard
                          event={event}
            ...
            {featuredEvents.length > 0 && (
              <div className="mb-32">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-1 h-8 bg-gradient-to-b from-purple-400 to-indigo-500 rounded-full"></div>
                  <h2 className="text-xl sm:text-2xl font-toroka font-medium text-gray-900">Featured Events</h2>
                  <span className="flex gap-1 text-sm text-gray-500 font-medium">
                    <Logo variant="icon"  className="text-indigo-500" />
                    <span>Zenvy Featured</span>
                  </span>
                </div>
                <div className="overflow-x-auto pb-4 featured-scrollbar">
                  <div className="flex gap-6 min-w-max">
                    {featuredEvents.map((event: any, idx: number) => (
                      <div key={`featured-${event._id}-${idx}`} className="flex-shrink-0 w-72">
                        <EventCard
                          event={event}
                          onClick={() => setSelectedEventId(event._id)}
                          isFeatured={true}
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl sm:text-2xl font-toroka font-medium text-gray-900">
              {activeFilters.length > 0 ? 'Filtered Events' : 'All Events'}
            </h2>
            <span className="text-sm text-gray-500 font-medium">
              {activeFilters.length > 0 ? 'Search results' : 'Browse everything'}
            </span>
          </div>

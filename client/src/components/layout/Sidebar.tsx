  ...
  return (
    <>
      ...
      <div className="lg:hidden flex items-center justify-between p-4 border-b border-slate-50 w-full">
        ...
        {user ? (
          <div onClick={() => user?.role === 'host' ? router.push('/host/profile') : ''} className={`w-8 h-8 rounded-full bg-slate-100 overflow-hidden border border-slate-200 ${user.role === 'host' ? 'cursor-pointer' : ''}`}>

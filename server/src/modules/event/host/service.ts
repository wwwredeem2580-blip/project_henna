...
export const updateEventService = async (req: Request, res: Response) => {
  try {
    ...
    if (!isValidObjectId(eventId as string) || !isValidObjectId(hostId as string)) {
      return res.status(400).json({ message: "Invalid event ID or host ID" });
    }
...
// Submit Event
export const submitEventService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
...
// Pending Event
export const updatePendingEventService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
...
// Approved Event
export const updateApprovedEventService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
...
// Published Event
export const updatePublishedEventService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
...
// Live Event
export const updateLiveEventService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
...
export const deleteEventService = async (req: Request, res: Response) => {
  try {
    ...
    if (event.status !== 'draft') {
      return res.status(400).json({ message: "Event is not in draft state" });
    }
...
export const updateStatusRejectedToDraftService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
    ...
    if (event.status !== 'rejected') {
      return res.status(400).json({ message: "Event is not in rejected state" });
    }
...
export const toggleSalesStatusService = async (req: Request, res: Response) => {
  try {
    const eventId = req.params?.eventId;
    const hostId = req.user?.sub;
    const pauseReason = req.body.reason;

    if(pauseReason?.length > 255) {
      return res.status(400).json({ message: "Reason must be less than 255 characters" });
    }
    ...
    if(event?.moderation?.sales?.paused && event?.moderation?.sales?.pausedBy) {
      if(event.moderation.sales.pausedBy.toString() !== hostId?.toString()) {
        return res.status(403).json({ message: `Sales are paused by admin for reason: ${event.moderation.sales?.pausedReason}` });
      }
    }
